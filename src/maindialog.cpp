...
MainDialog::MainDialog(QString userConfigFile) {
  ...
  if(config_read_file(&config_, userConfigFile_.toLocal8Bit().constData()) == CONFIG_FALSE) {
    ...
    qDebug() << "load fail, try " << PROJECT_DATA_DIR << "/picom.conf.example";
    config_read_file(&config_, PROJECT_DATA_DIR "/picom.conf.example");
  }
  ...
  ui->aboutButton->setIcon(QIcon::fromTheme("help-about", QIcon(":/help-about")));
  ui->buttonBox->button(QDialogButtonBox::StandardButton::Close)->setIcon(QIcon::fromTheme("application-exit", QIcon(":/application-exit")));
}
